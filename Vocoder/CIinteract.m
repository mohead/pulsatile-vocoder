% % function [out_signal,mspatial_weight] = CIinteract(signal,m, distance_electrodes, lambda)
...
for ii = 1:size(signal,1); %Over all channels
    ...
    out_signal(:,channel_active_idx) = out_signal(:,channel_active_idx)+bsxfun(@times, signal(ii,channel_active_idx),mspatial_weight(ii,:)');
end
end
