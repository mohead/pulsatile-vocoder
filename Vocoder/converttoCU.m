    ...
    % Threshold (TCL) and maximum (MCL) Clinical Units [CU] using Volume [0 1]. 
    % The mapping function used is described in page 68 of
    % Brett Swansons Dissertation "Pitch percept with cochlear implants",
    % equation 4.26 in
            TCL = TCL(:); MCL = MCL(:); %Make both row vectors
            assert(all(TCL(TCL ~= 0) < MCL(MCL ~= 0)), 'TCL must be smaller than MCL!');
            assert(isequal(find(TCL),find(MCL)),'TCL and MCL must be specified for the same electrodes');
